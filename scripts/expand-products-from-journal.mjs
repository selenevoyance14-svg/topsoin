...
const targetByCategory = { lingerie: 4, nuit: 4, sensualite: 4, erotisme: 6, soins: 5, cadeaux: 7 };
const selected = [];
for (const [cat, count] of Object.entries(targetByCategory)) selected.push(...candidates.filter((item) => item.cat === cat).slice(0, count));
if (selected.length < 30) selected.push(...candidates.filter((item) => !selected.includes(item)).slice(0, 30 - selected.length));
if (selected.length < 30) throw new Error(`Seulement ${selected.length} produits inédits trouvés dans le journal`);

let nextId = Math.max(...products.map((product) => product.id)) + 1;
for (const item of selected.slice(0, 30)) {
  products.push({ id: nextId++, cat: item.cat, name: item.name, sub: item.sub, price: item.price, was: "", off: "", rating: 0, reviews: 0, prime: false, tag: "Sélection Journal", color: colors[item.cat], asin: item.asin, image: item.image, url: item.url });
}
...
console.log(`${selected.slice(0, 30).length} produits du Journal ajoutés — ${products.length} produits au total.`);
